...
Applications are separate. The apply form sends structured facts and file bytes to `POST /apply` over HTTPS. The worker encrypts each file with AES-GCM before insert. It stores the ciphertext, the IV, and the label `AES-GCM`. It stores the typed facts needed for the rule check (profession, certificate names, language, experience years, qualification country, consent flags, matched rule id). It does not store the candidate email. It does not decrypt files while matching.

## Endpoints

- `POST /view` — declaration page opened (`erklaerung_view`)
- `POST /submit` — form consent (`consent_given`), returns `receipt_ref`
- `POST /withdraw` — restricts every consent row with that receipt and deletes the application plus ciphertext for that receipt. Unknown receipts return 404. This does not delete FormSubmit or AgentMail messages.
- `POST /pageview` — minimized page view (`page_visit`)
- `POST /apply` — encrypted application. Returns the receipt and whether a rule matched. It does not return employer personal data, ciphertext, or the candidate email. A missing `DATA_ENCRYPTION_KEY` rejects the upload and stores nothing.
- `GET /stats` — aggregate counts only. Header `Authorization: Bearer <STATS_TOKEN>`. Query-string tokens are rejected with HTTP 400.
- `GET /matches` — employer-facing matches only (share consent on and a rule matched). Same bearer token as `/stats`. Each row is match id, rule id, profession, consent flags, receipt, sample flag, and created time. No ciphertext and no email.
...
Do not write either value into git, into wrangler `[vars]`, or into the static site. The worker reads `env.STATS_TOKEN` and `env.DATA_ENCRYPTION_KEY`. If the stats token is missing, `/stats` and `/matches` return 401. If the encryption key is missing or not 32 bytes, `/apply` returns 503 and stores nothing.
...
Consent rows, application rows, and encrypted file rows older than six calendar months are deleted by:
...
Existing database. Run each file once. `001` adds columns and does not drop legacy email or ip_hash columns. `002` adds the application tables.
...
New consent writes leave `email`, `ip_hash`, `user_agent`, `referrer`, and `session_id` null. `payload_json` is `{}`. Application tables have no email column and no IP column.
